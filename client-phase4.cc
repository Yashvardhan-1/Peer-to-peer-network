    ...
    for( auto filePtr = master.search1.begin(); filePtr != master.search1.end(); ++filePtr ){
       ...
       // cout<<"### checking for file: "<<*filePtr<<" in neighbStore\n";
        ...
        if (found1)
        {
            ...
            output = "Found " + *filePtr + " at " + to_string(onePosseing) + " with MD5 0 at depth 1";
            cout << output << endl;
            ...
            output = "Found " + *filePtr + " at " + to_string(onePosseing) + " with MD5 0 at depth 2";
            cout << output << endl;
        }
        ...
        for (int i = 0; i < MAX_NEIGH; i++)
        {
            ...
            if (FD_ISSET(sd, &read_fds))
            {
                ...
                else
                {
                    ...
                    //cout<<"! "<<BUF<<" printing buffer in server of "<<master->CLIENT_ID<<endl;
                       ...
                       // cout<<"$$$ downlaod request for "<<buf<<endl;
                        ...
                        if( itr != master->fileAndPath.end() ){
                           ...
                           // cout<<"$$$ sending file "<<buf<< " of size "<< file_size <<endl;
                            ...
                            string size_string = to_string(file_size); 
                            char const* size = size_string.c_str();
...
void download_file_thread_func(Master* master, NeighSocket* neigh, string file){
   ...
   // cout<<"$ "<<neigh->private_ID<<endl;
    ...
    {    
        ...
        int file_size;
        char size[BUFF_SIZE];
        errorHandler(recv(client_socket, size, BUFF_SIZE, 0), (char*)"Recieve file size error");
        file_size = atoi(size);
        sleep(2);
